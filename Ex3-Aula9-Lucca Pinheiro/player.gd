...
func _ready() -> void:
	...
	# Trava o mouse no centro da tela (FPS)
...
func _physics_process(delta: float) -> void:
	...
	var wish_dir := _get_movement_direction()
	...
	# 5) MOVIMENTAÇÃO COM COLISÃO
	move_and_slide()

	# 6) ROTAÇÃO DO PLAYER/CÂMERA
...
func _get_movement_direction() -> Vector3:
	# Entrada WASD / Setas → "ui_up / ui_down / ui_left / ui_right"
	...
	# Movimento relativo à rotação do Player (como jogos FPS)
